...
Dự án đề xuất xây dựng một nền tảng web/app cho phép người dùng \textbf{thuê thiết bị gaming trong vài ngày hoặc thử trước khi mua}. Người dùng có thể tìm kiếm thiết bị theo loại sản phẩm, thương hiệu, game phù hợp, giá thuê, tình trạng thiết bị và thông số kỹ thuật. Sau đó, người dùng đặt thuê, thanh toán phí thuê, đặt cọc hoặc đăng ký hỗ trợ cọc, rồi nhận thiết bị để trải nghiệm thực tế.

Về dòng tiền, Mutux đóng vai trò trung gian giữa người thuê và người cho thuê. Nền tảng thu phí thuê, giữ cọc như một khoản bảo chứng, chuyển tiền thuê cho người cho thuê sau khi trừ hoa hồng. Với người dùng chưa đủ tiền cọc cho thiết bị có giá trị cao, Mutux có thể cung cấp dịch vụ hỗ trợ cọc: người thuê trả thêm một khoản phí dịch vụ, còn nền tảng đứng ra bảo chứng phần cọc theo điều kiện xác minh.
...
\begin{figure}[h]
    ...
    \begin{tikzpicture}[
        ...
        \node[box] (renter) at (0,1.6) {Người thuê\\game gear};
        \node[box] (platform) at (5.0,1.6) {Nền tảng\\Mutux};
        \node[box] (owner) at (10.0,1.6) {Người cho thuê\\shop/chủ gear};
        \node[smallbox] (deposit) at (5.0,-0.2) {Giữ cọc / hỗ trợ cọc\\hoàn cọc khi đủ điều kiện};

        \draw[money] ([yshift=0.22cm]renter.east) -- ([yshift=0.22cm]platform.west)
            node[midway, above=0.12cm, note]{Phí thuê + tiền cọc};
        \draw[money] ([yshift=-0.22cm]renter.east) -- ([yshift=-0.22cm]platform.west)
            node[midway, below=0.12cm, note]{hoặc phí hỗ trợ cọc};
        \draw[money] ([yshift=0.22cm]platform.east) -- ([yshift=0.22cm]owner.west)
            node[midway, above=0.12cm, note]{Tiền thuê sau hoa hồng};
        \draw[money] (platform.south) -- (deposit.north)
            node[midway, right=0.12cm, note]{quản lý cọc};

        \draw[flow] (10.0,-2.0) -- (0,-2.0)
            node[midway, above=0.12cm, note]{Giao thiết bị};
        \draw[flow] (0,-2.6) -- (10.0,-2.6)
