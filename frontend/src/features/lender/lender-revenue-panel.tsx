...
function Transaction({ transaction: tx }: { transaction: WalletTransaction }) { const outgoing = isOutgoingWalletTransaction(tx.type); const timestamp = tx.created_at || tx.createdAt; return <article className="flex flex-col gap-2 p-4 sm:flex-row sm:justify-between"><div className="flex items-center gap-3"><span className={`flex h-9 w-9 items-center justify-center rounded-v-md ${outgoing ? "bg-rose-500/10 text-rose-400" : "bg-emerald-500/10 text-emerald-400"}`}>{outgoing ? <ArrowUpRight size={17} /> : <ArrowDownLeft size={17} />}</span><div><p className="text-sm font-semibold">{getWalletTransactionLabel(tx.type)}</p><p className="text-xs text-vanguard-light-textMuted dark:text-vanguard-dark-textMuted">{tx.reference || tx.id}</p></div></div><div className="pl-12 text-left sm:pl-0 sm:text-right"><p className={`font-bold tabular-nums ${outgoing ? "text-rose-400" : "text-emerald-400"}`}>{outgoing ? "-" : "+"}{formatCurrency(Math.abs(toAmount(tx.amount)))}</p>{timestamp && <time dateTime={timestamp} className="text-xs text-vanguard-light-textMuted dark:text-vanguard-dark-textMuted">{new Date(timestamp).toLocaleString("vi-VN")}</time>}</div></article>; }
