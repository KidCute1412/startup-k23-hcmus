...
const CONDITION_PRESETS = [
  { value: "Like new, đã kiểm định", label: "Like New", desc: "Đã kiểm định, ngoại hình như mới" },
  { value: "Xuất sắc, chỉ dùng văn phòng", label: "Xuất sắc", desc: "Chỉ dùng văn phòng, cực kỳ giữ gìn" },
  { value: "Rất tốt, pad mới vệ sinh", label: "Rất tốt", desc: "Ngoại hình đẹp, đã vệ sinh kĩ càng" },
  { value: "Tốt, đã hấp vệ sinh", label: "Tốt", desc: "Hoạt động hoàn hảo, đã khử khuẩn" },
  { value: "Khá tốt, có vết dùng nhỏ", label: "Khá tốt", desc: "Có trầy xước nhẹ do sử dụng" },
];
...
export function AddGearForm() {
  ...
  const [condition, setCondition] = useState(CONDITION_PRESETS[0].value);
    ...
    try {
      ...
      specs.forEach((s) => {
        if (s.label.trim() && s.value.trim()) {
          specObj[s.label.trim()] = s.value.trim();
        }
      });

      const categoryMap: Record<string, string> = {
        keyboards: "20000000-0000-0000-0000-000000000003",
        mice: "20000000-0000-0000-0000-000000000002",
        audio: "20000000-0000-0000-0000-000000000004",
        setups: "20000000-0000-0000-0000-000000000006",
      };
              ...
              return (
                <li key={s.key} className="flex flex-1 items-center">
                  <button
                    ...
                    className={`flex items-center gap-2 font-display text-[18px] font-bold uppercase tracking-widest transition-colors ${current
                  ...
                  >
                    <span
                      className={`flex size-6 items-center justify-center rounded-full text-[15px] font-bold transition-colors ${current
        ...
        <div className="px-6 py-8">
              ...
              <div className="space-y-3">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  {CONDITION_PRESETS.map((preset) => {
                    const selected = !isCustomCondition && condition === preset.value;
                    return (
                      <button
                        key={preset.value}
                        type="button"
                  ...
                  >
                    ...
                    <span className="text-xs text-vanguard-light-textMuted dark:text-vanguard-dark-textMuted">
                      Nhập tình trạng chi tiết theo ý bạn
                    </span>
                  </button>
                </div>
                ...
                {isCustomCondition && (
                  <div className="mt-2.5 space-y-1.5 animate-fadeIn">
                    <Input
                      id="gear-condition-custom"
                      placeholder="VD: 98%, phím Space hơi lỏng nhẹ, đã thay keycap..."
                      value={customConditionText}
                      onChange={(e) => {
                        const val = e.target.value;
                        setCustomConditionText(val);
                        setCondition(val || "Tự định nghĩa");
                      }}
                      required
                    />
                  </div>
          ...
          {step === "preview" && (
            <div className="space-y-6">
              ...
              {submitError && (
                <div className="rounded-v-sm border border-red-500/30 bg-red-500/10 p-3 text-xs text-red-500">
                  {submitError}
                </div>
              )}
