...
export function DisputeResolutionFeature() {
  ...
  const disputes: DisputeCase[] = (rawDisputes || []).map((item) => {
    ...
    return {
      ...
      renter: {
        ...
        proof_images: renterEvidences.map((e) => resolveMediaUrl(e.url)),
      },
      lender: {
        id: lenderId || "",
        ...
        proof_images: lenderEvidences.map((e) => resolveMediaUrl(e.url)),
      },
      status: item.status,
  ...
  // Always update activeCase with refreshed item matching same ID upon refetch
  useEffect(() => {
  ...
  const handleResolveSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await resolveDispute(activeCase.id, {
        resolutionType,
        deductAmount: resolutionType === "deposit_deduct" ? deductAmount : undefined,
        resolutionNote,
      });
              ...
              <div className="rounded-v border border-vanguard-light-border bg-vanguard-light-surf p-6 shadow-md dark:border-vanguard-dark-border dark:bg-vanguard-dark-surf">
                ...
                <div className="mt-6 grid grid-cols-1 gap-6 md:grid-cols-2">
                  ...
                  <div className="rounded-v-sm border border-blue-500/30 bg-blue-500/5 p-4">
                    ...
                    {activeCase.renter.proof_images.length > 0 ? (
                      <div className="mt-3 grid grid-cols-2 gap-2">
                        {activeCase.renter.proof_images.map((src, idx) => (
                  ...
                  <div className="rounded-v-sm border border-purple-500/30 bg-purple-500/5 p-4">
                    ...
                    {activeCase.lender.proof_images.length > 0 ? (
                      <div className="mt-3 grid grid-cols-2 gap-2">
                        {activeCase.lender.proof_images.map((src, idx) => (
                          <div key={idx} className="relative h-28 w-full overflow-hidden rounded-v-sm bg-black">
                            <Image
                              src={src}
                    ...
                    <div className={`rounded-v-sm border p-5 ${
                      ...
                      {activeCase.resolution?.resolved_at && (
                        <div className="mt-3 text-[11px] text-vanguard-light-textMuted dark:text-vanguard-dark-textMuted">
                          Thời điểm phân xử: {new Date(activeCase.resolution.resolved_at).toLocaleString()}
                        </div>
                      )}
                    </div>
                  </div>
