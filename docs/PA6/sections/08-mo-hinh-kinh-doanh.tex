% !TeX root = ../main.tex
\section{Mô hình kinh doanh}

\subsection{Mô hình hoạt động cơ bản của dịch vụ}

Mô hình hoạt động của hệ thống Mutux được thiết kế nhằm đảm bảo sự minh bạch, an toàn và tiện lợi cho cả Người thuê (Renter) và Người cho thuê (Lender), dưới sự giám sát của Quản trị viên (Admin).

\subsubsection{Sơ đồ Use Case (Use Case Diagram)}

Sơ đồ Use Case mô tả các chức năng mà mỗi tác nhân có thể sử dụng trên nền tảng Mutux. Người dùng có thể tham gia với vai trò Người thuê (Renter), Người cho thuê (Lender), hoặc đảm nhiệm cả hai vai trò. Người cho thuê kế thừa toàn bộ use case của Người thuê và có thêm các chức năng quản lý tài sản cho thuê. Quản trị viên (Admin) là tác nhân kiểm soát các hoạt động cần phê duyệt và xử lý ngoại lệ.

\paragraph{Use case của Người thuê (Renter).}
\begin{itemize}
    \item Xem danh mục thiết bị.
    \item Quản lý giỏ hàng.
    \item Đổi mật khẩu.
    \item Xóa tài khoản.
    \item Yêu cầu xác thực KYC.
    \item Quản lý sổ địa chỉ.
    \item Quản lý lịch sử đơn thuê.
    \item Quản lý ví tiêu dùng.
    \item Quản lý ví trả sau.
    \item Đánh giá người cho thuê, thiết bị.
\end{itemize}

\paragraph{Use case của Người cho thuê (Lender).}
Người cho thuê được sử dụng toàn bộ use case của Người thuê và có thêm các use case sau:
\begin{itemize}
    \item Nạp tiền vào Ví Mutux.
    \item Yêu cầu hạn mức tín dụng.
    \item Quản lý đơn thuê.
    \item Đăng sản phẩm cho thuê.
    \item Đánh giá người thuê.
\end{itemize}

\paragraph{Use case của Quản trị viên (Admin).}
\begin{itemize}
    \item Duyệt hạn mức tín dụng.
    \item Duyệt KYC.
    \item Duyệt thiết bị.
    \item Xử lý tranh chấp.
\end{itemize}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\textwidth]{../PA6/figures/usecase.png}
    \caption{Sơ đồ Use Case của hệ thống Mutux}
    \label{fig:usecase}
\end{figure}

\subsubsection{Sơ đồ Hoạt động (Activity Diagram)}

Quy trình cốt lõi nhất của Mutux là luồng Đặt thuê và Bàn giao thiết bị. Hệ thống áp dụng cơ chế thanh toán tạm giữ (Escrow) và quy trình xác thực minh chứng (Proof) để bảo vệ cả hai bên.

Quy trình cơ bản diễn ra theo các bước sau:
\begin{enumerate}
    \item \textbf{Đặt thuê:} Người thuê tìm kiếm thiết bị, chọn thời gian thuê, địa chỉ giao nhận và gửi đơn từ giỏ hàng.
    \item \textbf{Người thuê xác nhận:} Người thuê kiểm tra lại thông tin đơn, phương thức thanh toán và xác nhận yêu cầu thuê. Người cho thuê tiếp nhận, xác nhận khả năng đáp ứng và chuẩn bị giao thiết bị.
    \item \textbf{Khóa cọc và trừ phí thuê:} Khi đơn được xác nhận, hệ thống khóa tiền cọc và trừ phí thuê từ ví tiêu dùng hoặc ví trả sau theo lựa chọn của người thuê. Khoản tiền được ghi nhận trong Escrow, chưa chuyển ngay cho người cho thuê.
    \item \textbf{Giao nhận:} Hai bên thực hiện giao nhận theo lịch hẹn. Người cho thuê cập nhật minh chứng bàn giao; người thuê kiểm tra tình trạng thiết bị và xác nhận đã nhận hàng trên hệ thống.
    \item \textbf{Hoàn trả:} Khi kết thúc thời gian thuê, người thuê hoàn trả thiết bị và gửi minh chứng. Người cho thuê kiểm tra, xác nhận tình trạng nhận lại hoặc mở yêu cầu tranh chấp nếu phát hiện hư hỏng, thiếu hụt hay mất mát.
    \item \textbf{Quyết toán:} Nếu không có tranh chấp, hệ thống hoàn cọc cho người thuê và chuyển tiền thuê sau khi trừ phí nền tảng vào Ví Mutux của người cho thuê. Nếu có tranh chấp, khoản tiền liên quan được giữ lại để Admin xem xét và quyết định.
    \item \textbf{Đánh giá:} Sau khi quyết toán hoàn tất, người thuê và người cho thuê đánh giá lẫn nhau cùng chất lượng giao dịch. Kết quả được lưu vào hồ sơ để hỗ trợ xây dựng mức độ tin cậy trên nền tảng.
\end{enumerate}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\textwidth]{../PA6/figures/activity.png}
    \caption{Sơ đồ Hoạt động quy trình thuê thiết bị trên Mutux}
    \label{fig:activity}
\end{figure}

\subsection{Mô hình doanh thu của dự án}
Mutux vận hành theo mô hình trung gian kết nối (Marketplace) kết hợp dịch vụ bảo vệ thanh toán (Escrow). Thay vì phụ thuộc hoàn toàn vào phí thuê thiết bị – phần doanh thu vốn dĩ thuộc về đối tác cho thuê – Mutux đa dạng hóa nguồn doanh thu thông qua các giao dịch B2C, các dịch vụ dành cho đối tác B2B và các chương trình hợp tác với đối tác thanh toán. Mô hình này giúp tối ưu hóa biên lợi nhuận thông qua việc khai thác luồng giao dịch từ cả hai phía cung và cầu.


\subsection{Các nguồn doanh thu dự kiến}

\textbf{Nguồn doanh thu cơ sở (từ Giao dịch B2C):}
\begin{itemize}
    \item \textbf{Phí nền tảng (Take Rate):} Đây là nguồn thu chủ lực của Mutux. Hệ thống sẽ tự động trích xuất hoa hồng \textbf{30\%} trên tổng giá trị tiền thuê của mỗi giao dịch thành công.
    \item \textbf{Phí bảo hiểm:} Người thuê có thể lựa chọn gói bảo hiểm tự nguyện với một khoản phí nhỏ để được miễn hoặc giảm chi phí bồi thường đối với các hư hỏng nhẹ ngoài ý muốn theo điều kiện áp dụng.
    \item \textbf{Phí giới thiệu mua sản phẩm:} Sau khi thuê và quyết định mua sản phẩm tại shop đối tác, Mutux nhận hoa hồng giới thiệu theo tỷ lệ phần trăm hoặc một khoản cố định trên mỗi đơn hàng thành công.
\end{itemize}

\textbf{Nguồn doanh thu Dịch vụ (Theo lộ trình 2 giai đoạn):}
\begin{itemize}
    \item \textbf{Dịch vụ Hỗ trợ tiền cọc (Deposit Advance Service):} Trong giai đoạn đầu, Mutux triển khai cơ chế hỗ trợ ứng trước tiền cọc cho một số khách hàng đáp ứng điều kiện xác minh danh tính (eKYC) và tiêu chí đánh giá nội bộ. Nền tảng thu \textbf{phí dịch vụ là 30.000 VNĐ} cho mỗi tháng người dùng sử dụng tiền cọc để nạp vào ví. Dịch vụ này sử dụng mô hình hợp tác với các đối tác tài chính (BNPL như MoMo, Fundiin) để giảm áp lực vốn lưu động và mở rộng khả năng phục vụ. Mutux không trực tiếp cung cấp dịch vụ tín dụng. Trong giai đoạn đầu, nền tảng chỉ đóng vai trò kết nối người dùng với đối tác BNPL hoặc các tổ chức tài chính. Mutux thu hoa hồng giới thiệu hoặc phí dịch vụ từ đối tác. Đối với rủi ro hỏng hóc/mất mát thiết bị, Mutux áp dụng cơ chế Giữ tiền Escrow + Định danh eKYC + Trừ trực tiếp tiền cọc/ví đối soát nếu xảy ra tranh chấp.
\end{itemize}

\textbf{Nguồn doanh thu mở rộng (từ Đối tác B2B - Triển khai sau 1 năm):}
\begin{itemize}
    \item \textbf{Phí quảng cáo và Đẩy tin (Featured Listings):} Nhằm tối ưu hóa khả năng bán hàng cho các shop gaming gear, Mutux thu phí để hiển thị ưu tiên sản phẩm của họ trên trang chủ hoặc khi người dùng tìm kiếm. 
    \item \textbf{Bán hộ gear cũ:} Mutux hỗ trợ shop hoặc cá nhân kiểm định, đăng bán, thanh toán và giao dịch thiết bị đã qua sử dụng hoặc hàng demo sau thời gian cho thuê; nền tảng thu hoa hồng trên giá bán khi giao dịch thành công.
\end{itemize}




\subsection{Phân tích mô hình phí và vận hành}

\subsubsection{Cách hoạt động và các khoản phí}

Mutux hoạt động theo mô hình marketplace kết nối ba bên chính: người thuê gear, shop hoặc chủ gear, và nền tảng Mutux. Trong phiên bản mở rộng, Mutux có thể kết nối thêm đối tác tài chính để triển khai dịch vụ Cọc Trả Sau, tức hỗ trợ vay hoặc bảo chứng phần cọc theo giá trị sản phẩm.

Các khoản phí chính trong mô hình gồm:

\begin{itemize}
    \item \textbf{Phí thuê gear}: người thuê trả theo ngày hoặc theo gói ngắn hạn. Ví dụ, một tay cầm trị giá khoảng 2.000.000 đồng có thể được cho thuê với giá 30.000 đồng/ngày; nếu thuê ba ngày thì phí thuê là 90.000 đồng.
    \item \textbf{Hoa hồng nền tảng}: Mutux lấy khoảng 20\%--30\% trên phí thuê thành công. Nếu phí thuê là 30.000 đồng, nền tảng có thể nhận 3.000--6.000 đồng, phần còn lại thuộc về shop hoặc chủ gear.
    \item \textbf{Tiền cọc truyền thống}: người thuê đặt cọc để bảo vệ tài sản cho bên cho thuê. Với thiết bị trị giá khoảng 1.800.000--2.000.000 đồng, tiền cọc có thể bằng toàn bộ giá trị sản phẩm. Khoản này không phải doanh thu của Mutux mà là khoản đảm bảo, được hoàn lại khi người thuê trả thiết bị đúng tình trạng.
    \item \textbf{Phí Cọc Trả Sau hoặc phí vay cọc}: nếu người dùng không muốn hoặc không đủ khả năng ứng trước toàn bộ tiền cọc, họ có thể chọn dịch vụ hỗ trợ cọc. Ví dụ, thay vì khóa trước 2.000.000 đồng tiền cọc cho một món gear, người dùng trả thêm khoảng 30.000 đồng phí vay khi giao dịch lần đầu tiên trong tháng. Mức phí này được thiết kế rất thấp, tương tự các nền tảng ví trả sau hoặc SPayLater, còn tiền thuê/mượn thiết bị vẫn do người cho thuê đặt ra.
    \item \textbf{Hoa hồng từ đối tác tài chính}: nếu dịch vụ Cọc Trả Sau được triển khai qua ngân hàng, ví điện tử hoặc công ty tài chính được cấp phép, Mutux có thể nhận phần chia sẻ doanh thu hoặc hoa hồng giới thiệu từ giao dịch này.
    \item \textbf{Phí bảo hiểm thiết bị}: người thuê trả thêm phí nếu lựa chọn gói bảo hiểm tự nguyện cho thiết bị trong thời gian thuê; khoản phí này tạo doanh thu cho Mutux.
    \item \textbf{Phí giới thiệu mua sản phẩm}: nếu người dùng thuê thử và quyết định mua sản phẩm tại shop đối tác, Mutux nhận hoa hồng theo tỷ lệ phần trăm hoặc khoản cố định trên đơn hàng thành công. Shop chỉ trả phí khi bán được hàng.
    \item \textbf{Hoa hồng bán hộ gear cũ}: Mutux thu hoa hồng khi bán thành công gear đã qua sử dụng hoặc hàng demo cho shop, cá nhân và đối tác cho thuê.
    \item \textbf{Phí quảng cáo và đẩy tin}: shop trả phí để ưu tiên hiển thị sản phẩm trên trang chủ hoặc kết quả tìm kiếm \textit{(nguồn doanh thu tương lai)}.
    \item \textbf{Gói thành viên}: người dùng trả phí định kỳ để được giảm phí thuê, giảm phí bảo lãnh cọc, ưu tiên thuê gear hot hoặc nhận ưu đãi giao nhận \textit{(nguồn doanh thu tương lai)}.
    \item \textbf{Gói dịch vụ cho shop}: shop trả phí để đăng nhiều thiết bị hơn, sử dụng dashboard quản lý đơn thuê, quảng bá sản phẩm hoặc tiếp cận nhóm khách có nhu cầu mua sau khi dùng thử \textit{(nguồn doanh thu tương lai)}.
\end{itemize}

Với cách vận hành này, doanh thu của Mutux không phụ thuộc hoàn toàn vào phí thuê gear. Phần phí thuê chủ yếu cần đủ hấp dẫn để shop và chủ gear có động lực đưa thiết bị lên nền tảng. Lớp doanh thu bổ sung nằm ở phí bảo hiểm, phí giao nhận, phí giới thiệu mua sản phẩm, hoa hồng bán hộ gear cũ, phí vay/hỗ trợ cọc, chia sẻ doanh thu với đối tác tài chính và các nguồn thu tương lai như quảng cáo, gói thành viên và gói dịch vụ cho shop.

\subsubsection{So sánh hai cách đặt cọc}

Điểm khác biệt quan trọng của Mutux là cho người dùng lựa chọn giữa đặt cọc truyền thống và Cọc Trả Sau. Ví dụ, người dùng thuê một tay cầm trị giá 2.000.000 đồng trong ba ngày, với phí thuê 10.000 đồng/ngày.

\begin{center}
\begin{tabular}{>{\raggedright\arraybackslash}p{0.36\textwidth} >{\raggedleft\arraybackslash}p{0.22\textwidth} >{\raggedleft\arraybackslash}p{0.24\textwidth}}
\toprule
\textbf{Khoản mục} & \textbf{Cọc truyền thống} & \textbf{Cọc Trả Sau} \\
\midrule
Giá trị tay cầm & 2.000.000đ & 2.000.000đ \\
Phí thuê 3 ngày & 90.000đ & 90.000đ \\
Tiền cọc ứng trước & 2.000.000đ & 0đ \\
Phí vay/hỗ trợ cọc & 0đ & 0--30.000đ \\
Tổng tiền ban đầu & 2.090.000đ & 90.000--110.000đ \\
\bottomrule
\end{tabular}
\end{center}

Như vậy, với cùng một món tay cầm trị giá 2.000.000 đồng, cách thuê truyền thống khiến người dùng phải chuẩn bị 2.090.000 đồng ngay từ đầu, còn Cọc Trả Sau chỉ cần khoảng 90.000--110.000 đồng cho ba ngày thuê. Khoản chênh lệch này làm mô hình đủ tiện và hợp lý hơn cho sinh viên, người mới chơi eSports hoặc người có ngân sách hạn chế: họ vẫn được dùng thử thiết bị thật, bên cho thuê vẫn có cơ chế bảo vệ tài sản nếu hư hỏng hoặc mất đồ, còn Mutux có thêm nguồn thu ngoài hoa hồng thuê.

\subsubsection{Tính khả thi về vận hành}

Ở giai đoạn MVP, Mutux chưa cần tự đứng ra cho vay hoặc triển khai sản phẩm tài chính thật. Hướng an toàn hơn là mô phỏng flow Cọc Trả Sau để chứng minh nhu cầu, sau đó mới hợp tác với ngân hàng, ví điện tử hoặc công ty tài chính được cấp phép nếu muốn triển khai thật.

Để mô hình vận hành được, nền tảng cần có các cơ chế cơ bản:

\begin{itemize}
    \item Xác minh danh tính người thuê trước khi cho thuê thiết bị có giá trị cao.
    \item Kiểm tra và ghi nhận tình trạng thiết bị trước và sau khi thuê.
    \item Quy định rõ phí trả muộn, phí hư hỏng, mất thiết bị và điều kiện hoàn cọc.
    \item Theo dõi trạng thái thiết bị: available, booked, renting, returned, maintenance.
    \item Giới hạn giá trị hỗ trợ cọc theo độ tin cậy của người thuê.
    \item Ưu tiên thử nghiệm với một số shop hoặc cộng đồng gamer nhỏ trước khi mở rộng.
\end{itemize}

Nhờ đó, Mutux có thể kiểm chứng nhu cầu thị trường trước, đồng thời tránh rủi ro pháp lý khi chưa đủ điều kiện tự cung cấp dịch vụ tài chính.
